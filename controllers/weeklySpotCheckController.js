import WeeklySpotCheck from "../models/WeeklySpotCheck.js";
import Warehouse from "../models/Warehouse.js";
import User from "../models/User.js";

// Create (auto-set warehouse)
export const createWeeklySpotCheck = async (req, res) => {
  try {
    ...
    const record = new WeeklySpotCheck({
      ...formData,
...
export const getMyWeeklySpotCheck = async (req, res) => {
  try {
    const records = await WeeklySpotCheck.find({ createdBy: req.userId })
...
export const getWeeklySpotCheck = async (req, res) => {
  try {
    ...
    const records = await WeeklySpotCheck.find({ warehouse: { $in: warehouseIds } })
...
export const verifyWeeklyspotcheck = async (req, res) => {
  try {
    const { id } = req.params;
    const { adminEmail } = req.body;

    const record = await WeeklySpotCheck.findById(id).populate("warehouse");
    ...
    const updated = await WeeklySpotCheck.findByIdAndUpdate(
      id,
...
export const deleteWeekySpotCheck = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await WeeklySpotCheck.findByIdAndDelete(id);
